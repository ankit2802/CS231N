...
class TwoLayerNet(object):
  ...
  def __init__(self, input_dim=3*256*256, hidden_dim=100, num_classes=10,
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*256*256, num_classes=10,
    ...
    for i in range(1, self.num_layers+1):
      if i == 1:
        ...
        input_layer_dim = hidden_dims[i-2]
  ...
  def loss(self, X, y=None):
    ...
    for i in range(1, self.num_layers):
        ...
        if self.use_batchnorm:
            ...
            key_beta = 'beta'+str(i)
            affine_out, fc_cache = affine_forward(current_input, self.params[keyW], self.params[keyb])
            bn_out, bn_cache = batchnorm_forward(affine_out, self.params[key_gamma], self.params[key_beta], self.bn_params[i-1])
