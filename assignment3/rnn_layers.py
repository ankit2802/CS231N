...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
  ...
  # step 1: from input to hidden layer
  xWx = np.dot(x, Wx)
  # step 2:hidden-hidden connections
  prev_hWh = np.dot(prev_h,Wh)
  # step 3:total with bias
  total_from_xWx_n_prev_hWh = xWx + prev_hWh + b.T
  # step 4:to next hidden state
  next_h = np.tanh(total_from_xWx_n_prev_hWh)
  cache = (x, prev_h.copy(), Wx, Wh,total_from_xWx_n_prev_hWh, next_h)
  ##############################################################################
  #                               END OF YOUR CODE                             #
  ##############################################################################
  return next_h, cache
...
def rnn_forward(x, h0, Wx, Wh, b):
  ...
  for t in xrange(T):
    if t==0:
        h[:,t,:], cache_step = rnn_step_forward(x[:,t,:], h0, Wx, Wh, b)
    else:
...
def rnn_backward(dh, cache):
  ...
  #we do not want to modify original dh object
  dh = dh.copy()
  for t in reversed(xrange(T)):
    dh[:,t,:]  += dprev_h # updating the last layer dh
    dx_local, dprev_h, dWx_local, dWh_local, db_local = rnn_step_backward(dh[:,t,:], cache[t])
    dx[:,t,:]  += dx_local
    dWx += dWx_local
    dWh += dWh_local
    db += db_local
...
def word_embedding_backward(dout, cache):
  ...
  x, W = cache
  dW = np.zeros_like(W)
  #it adds dout at each position of x
  np.add.at(dW, x, dout)
  ##############################################################################
  #                               END OF YOUR CODE                             #
  ##############################################################################
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
  ...
  #############################################################################
  #Similar to RNN we implement LSTM in steps as explained in the ipython notebok
  N,H = prev_c.shape
  ...
  cache = (x, prev_h, prev_c, a, a_i, a_f, a_o, a_g, next_h, next_c, Wx, Wh)
  ##############################################################################
  #                               END OF YOUR CODE                             #
  ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
  ...
  #############################################################################
  #Again we implement back pass as in RNN in steps
  (x, prev_h, prev_c, a, a_i, a_f, a_o, a_g, next_h, next_c, Wx, Wh) = cache
  N,H = dnext_h.shape
  da = np.zeros(a.shape)
  # step 7:
  dnext_c = dnext_c.copy()
  dnext_c = dnext_c + a_o * (1 - np.tanh(next_c) ** 2) * dnext_h
...
def lstm_forward(x, h0, Wx, Wh, b):
  ...
  for t in range(T):
    if t==0:
        h[:,t,:], c[:,t,:], cache_step = lstm_step_forward(x[:,t,:], h0, c0, Wx, Wh, b)
        cache.append(cache_step)
    else:
...
def lstm_backward(dh, cache):
  ...
  for t in reversed(xrange(T)):
    ...
    dc[:,t,:]  += dprev_c # updating the previous layer dh
    dx_local, dprev_h, dprev_c, dWx_local, dWh_local, db_local = lstm_step_backward(dh[:,t,:], dc[:,t,:], cache[t])
    dx[:,t,:]  += dx_local
    dWx += dWx_local
    dWh += dWh_local
    db += db_local
